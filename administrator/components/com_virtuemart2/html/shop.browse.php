...
echo '<div style="background-color:#F3E8E9; color:#000000; padding-left:30px;">';
...
elseif( $num_rows == 1 && ( !empty($keyword) || !empty($keyword1) ) ) {
	...
	while ($db_browse->next_record()) {
		...
		if (USE_AS_CATALOGUE != '1' && $product_price != "" 
			...
			$products[$i]['form_addtocart'] = $tpl->fetch( 'browse/includes/1addtocart_form.tpl.php' );
	...
	$recent_products = $ps_product->recentProducts(null,$tpl->get_cfg('showRecent', 0));
